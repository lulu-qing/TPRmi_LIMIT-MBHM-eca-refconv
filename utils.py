...
def count_acc_topk(logits, label, k=5):
    # 【新增安全判断】：如果当前模型的输出类别数小于 k，就把 k 缩小为类别数
    if logits.shape[1] < k:
        k = logits.shape[1]

    _, maxk = torch.topk(logits, k, dim=-1)
    maxk = maxk.t()
    correct = maxk.eq(label.view(1, -1).expand_as(maxk))

    # 根据实际的 k 值计算 top-k 准确率
    correct_k = correct[:k].reshape(-1).float().sum(0)
    acc = correct_k / label.size(0)
    return acc.item()
