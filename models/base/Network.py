...
# 请确保已创建此文件并包含 FCN_Encoder 类
from models.fcn_encoder import FCN_Encoder
...
class MYNET(nn.Module):
    ...
    def __init__(self, args, mode=None):
        ...
        if self.args.dataset in ['cifar100', 'manyshotcifar']:
        ...
        elif self.args.dataset in ['mini_imagenet', 'manyshotmini', 'imagenet100', 'imagenet1000',
            ...
            self.num_features = 512
        elif self.args.dataset == 'mbhm':
            # 使用针对 1D 振动信号的 FCN 编码器
            self.encoder = FCN_Encoder()
            self.num_features = 128  # FCN 最后一层输出 128 维特征
    ...
    def encode(self, x):
        ...
        if self.args.dataset == 'mbhm':
            # 针对 1D 信号 (Batch, Channels, Length) 进行池化
            x = F.adaptive_avg_pool1d(x, 1)
            x = x.squeeze(-1)
        else:
            # 针对 2D 图像 (Batch, Channels, H, W) 进行池化
