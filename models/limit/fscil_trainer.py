...
class FSCILTrainer(Trainer):
    ...
    #
    #     # 顶层随机初始化的分类器与多头注意力 (slf_attn)
    #     top_para = [v for k, v in self.model.named_parameters() if ('encoder' not in k and 'cls' not in k)]
    #
    #     # 🚀 核心修改：三级学习率架构
    #     optimizer = torch.optim.SGD([
    #         {'params': base_params, 'lr': self.args.lr_base},  # 第1级：0.0 (绝对冻结历史知识)
    #         {'params': refconv_params, 'lr': self.args.lrg},  # 第2级：0.0005 (微调精密的 ECA-RefConv)
    #         {'params': top_para, 'lr': 0.01}  # 第3级：0.01 (给随机初始化的顶层 20 倍的超大学习率，让它快速收敛！)
    #     ], momentum=0.9, nesterov=True, weight_decay=self.args.decay)
    #
    #     if self.args.schedule == 'Step':
    #         scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=self.args.step, gamma=self.args.gamma)
    ...
    def get_optimizer_base(self):
        ...
        # --- 终极奥义：全网锁定，只许 ECA-RefConv 学习！ ---
        optimizer = torch.optim.SGD([
            {'params': base_params, 'lr': 0.0},  # 绝对冻结骨干网络
            {'params': top_para, 'lr': 0.0},  # 绝对冻结顶层分类器 (95%的底座)
            {'params': refconv_params, 'lr': self.args.lrg}  # 0.0005：全村的希望，只训练重聚焦模块！
        ], momentum=0.9, nesterov=True, weight_decay=self.args.decay)
