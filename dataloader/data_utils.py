...
def set_up_datasets(args):
    ...
    if args.dataset == 'mbhm':
        import dataloader.mbhm.mbhm as Dataset
        args.base_class = 4  # 基础类 0, 2, 5, 8
        args.num_classes = 10
        args.way = 2  # 增量任务每次增加 2 类
        args.shot = 5
        args.sessions = 4  # 1(Base) + 3(Inc)
    # --------------------------
...
def get_base_dataloader(args):
    ...
    if args.dataset == 'mbhm':
        # 读取非连续类别索引 (0, 2, 5, 8)
        class_index = [int(line.strip()) for line in open(txt_path, 'r').readlines()]
        trainset = args.Dataset.MBHM(root=args.dataroot, train=True, index_path=txt_path, base_sess=True)
        testset = args.Dataset.MBHM(root=args.dataroot, train=False, index=class_index)
    else:
...
def get_base_dataloader_meta(args):
    ...
    if args.dataset == 'mbhm':
        class_index = [int(line.strip()) for line in open(txt_path, 'r').readlines()]
        trainset = args.Dataset.MBHM(root=args.dataroot, train=True, index_path=txt_path)
        testset = args.Dataset.MBHM(root=args.dataroot, train=False, index=class_index)
    else:
        class_index = np.arange(args.base_class)
        if args.dataset == 'cifar100':
...
def get_new_dataloader(args, session):
    ...
    if args.dataset == 'mbhm':
        testset = args.Dataset.MBHM(root=args.dataroot, train=False, index=class_new)
    else:
        if args.dataset == 'cifar100':
            testset = args.Dataset.CIFAR100(root=args.dataroot, train=False, download=False,
...
def get_session_classes(args, session):
    if args.dataset == 'mbhm':
        # 通过合并所有 session 的文本文件来支持非连续类别标签
        class_list = []
        for i in range(session + 1):
            txt_path = "data/index_list/" + args.dataset + "/session_" + str(i + 1) + ".txt"
